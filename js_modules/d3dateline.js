function loadchart(div, graph) {
   ...
   // d3.json(json, function (error, graph) {
        ...
        var interval = (latest - earliest) / 1000 / 60 / 60 / 24;
        // ... determines the width of the svg
        var width = interval * dayWidth;
        width = Math.max(width, dayWidth * 400); // min one year
		...
		*************************/
        ...
        var x = d3.time.scale()
            .domain([d3.time.day.offset(earliest, - 150), d3.time.day.offset(latest, 150)])
        ...
        function curve(d) {
            ...
            if (d.source.x == d.target.x) {
                ...
                var dx = d.target.x - d.source.x,
                    dy = Math.min(d.target.y - d.source.y, 1),
                    dr = Math.sqrt(dx * dx + dy * dy);
                if (dx > 1000) {
                    //console.log('I am here...', dx);
                    dr = 9e3;
                }
                // if (dx < 5) {
                //     dr = -10;
